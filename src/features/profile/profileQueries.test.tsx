...
import { isFollowingQuery, followUserQuery, getFollowing, getFollowers, getCount} from "./profileQueries";
// thunk
...
describe("getCount", () => {
    it("should return the number of followers and following", async () => {
        ...
        const followingCount = jest.fn(() => Promise.resolve([{ id: "1" }, { id: "2" }]));
        const followerCount = jest.fn(() => Promise.resolve([{ id: "1" }]));
        (DataStore.query as jest.Mock).mockImplementationOnce(followingCount).mockImplementationOnce(followerCount);
